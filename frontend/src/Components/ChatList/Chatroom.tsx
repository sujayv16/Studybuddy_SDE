...
const handleSubmit = (event: any) => {
  ...
  return (
    ...
    <>
      ...
      <Notifications socket={socket} currentChatId={id} currentUsername={data.username} />
      <Container>
        <Row>
          <Col md={9}>
            <div style={{ border: '1px solid gray', padding: '10px', borderRadius: '5px' }}>
              <div style={{ height: '80vh', overflowY: 'scroll' }} ref={messagesEndRef}>
                {messages.map((message: any, index: number) => {
                  console.log(`Message ${index}:`, { fromUser: message.fromUser, currentUser: data.username, isOwn: message.fromUser === data.username });
                  return (
