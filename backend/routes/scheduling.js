...
router.post('/courses', async (req, res) => {
  ...
  const { courseId, courseName, semester, year, priority, studyGoals } =
    req.body;
...
router.get('/find-partners/:courseId', async (req, res) => {
  ...
  try {
    ...
    const potentialPartners = courseEnrollments.map(
      (enrollment) => enrollment.username
    );
    ...
    if (day !== undefined && timeSlot) {
      ...
      partners = partners.filter((partner) =>
        availableUsernames.includes(partner.username)
      );
    }
...
function findCommonTimeSlots(dayAvailability, participants, durationMinutes) {
  ...
  baseSlots.forEach((baseSlot) => {
    ...
    for (let i = 1; i < participants.length; i++) {
      ...
      participantSlots.forEach((slot) => {
        if (slot.startTime < commonEnd && slot.endTime > commonStart) {
          ...
          commonStart =
            slot.startTime > commonStart ? slot.startTime : commonStart;
    ...
    if (
      isCommonSlot &&
      getTimeDifferenceInMinutes(commonStart, commonEnd) >= durationMinutes
    ) {
