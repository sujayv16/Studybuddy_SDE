...
const connectDB = async () => {
  try {
    ...
    if (mongoMode === 'none') {
      ...
      console.log(
        '💡 Set MONGO_MODE to "local" or "atlas" to enable database connection'
      );
    ...
    console.log(
      `🔄 Connecting to ${useLocalMongo ? 'Local MongoDB' : 'MongoDB Atlas'}...`
    );
    ...
    if (error.message.includes('ECONNREFUSED')) {
      ...
      console.log(
        '   1. Start MongoDB service: net start MongoDB (as administrator)'
      );
      console.log(
        '   2. Or run: "C:\\Program Files\\MongoDB\\Server\\8.2\\bin\\mongod.exe" --dbpath C:\\data\\db'
      );
      console.log(
        '   3. Or set MONGO_MODE=none in .env to skip database connection'
      );
    } else if (
      ...
      console.log(
        '   3. SSL/TLS version compatibility issues with your system'
      );
      console.log(
        '   4. Set MONGO_MODE=none in .env to skip database connection for testing'
      );
    }
